...
class MPNN(pl.LightningModule):
    ...
    def validation_step(self, batch: TrainingBatch, batch_idx: int = 0):
        losses, weighted_losses = self._evaluate_batch(batch, include_weighted=True)
    ...
    def _evaluate_batch(self, batch, include_weighted = False) -> list[Tensor]:
        bmg, V_d, X_d, targets, weights, lt_mask, gt_mask = batch
        ...
        one_weights = torch.ones_like(targets)
        ...
        if include_weighted == False:
            return [
                metric(preds, targets, mask, one_weights, lt_mask, gt_mask) for metric in self.metrics[:-1]
            ]
        else:
            return [metric(preds, targets, mask, one_weights, lt_mask, gt_mask) for metric in self.metrics[:-1]], [metric(preds, targets, mask, weights, lt_mask, gt_mask) for metric in self.metrics[:-1]]
